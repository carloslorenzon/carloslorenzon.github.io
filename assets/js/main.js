(function ($) {
    ...
    $(function () {
    ...
    }); $(window).on('load', function () { $(".preloader").fadeOut(800, "linear"); var ptPage = $('.animated-sections'); }).on('resize', function () { mobileMenuHide(); $('.animated-section').each(function () { $(this).perfectScrollbar('update'); }); customScroll(); }); $(document).on('ready', function () {
        ...
        // Itera sobre cada link e adiciona a classe 'active' se o caminho atual contiver o link
        if (window.location.pathname !== "" && window.location.pathname !== "/") {
            links.forEach(link => {
                link.classList.remove('active');
                if (link.href !== "" && link.getAttribute('href') !== "/" && window.location.href.startsWith(link.href)) {
                    console.log(link);
